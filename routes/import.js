...
router.post('/import/organization-roster', function(req, res, next){
	...
	if(req.files.uploadorganizationroster.mimetype == 'application/vnd.ms-excel'){
		...
		//Moved logic from current organization - excel report
		...
		for (var i = 0; i < data.length; i++){
			...
			if(typeof data[i] !== 'undefined'){
				...
				//No Zone Name = bad record
				if(!(typeof rawZoneName === 'undefined')){
					...
					areas[missionGroup] = sortObj(areas[missionGroup]);
					...
					areas[missionGroup][zoneName] = sortObj(areas[missionGroup][zoneName]);
					...
					areas[missionGroup][zoneName][data[i]['District']] = sortObj(areas[missionGroup][zoneName][data[i]['District']]);					
					...
					areas[missionGroup][zoneName][data[i]['District']][data[i]['Area']]['missionaries'].push(data[i]['Missionary Name']);
				}
			}
		}
		...
		res.render('import', { title: 'Areas', alert: {type: 'success', title: 'Upload successful!', body: 'Your upload of '+req.files.uploadorganizationroster.filename+' was successful!'} });
	}else{
		res.redirect('/import?success=false');
	}
